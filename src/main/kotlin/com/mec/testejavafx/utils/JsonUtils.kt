...
object JacksonExtension {
    val jacksonObjectMapper: ObjectMapper by lazy {
        ObjectMapper().registerModule(JavaTimeModule()).setSerializationInclusion(JsonInclude.Include.NON_NULL)
            .disable(SerializationFeature.WRITE_DATES_AS_TIMESTAMPS).registerModule(KotlinModule.Builder().build())
    }
}
...
fun <T> String.jsonToObject(t: Class<T>): T = JacksonExtension.jacksonObjectMapper.readValue(this, t)

inline fun <reified T> String.jsonToObject(): T =
    JacksonExtension.jacksonObjectMapper.readValue(this, jacksonTypeRef<T>())

fun <T> T.objectToJson(): String = JacksonExtension.jacksonObjectMapper.writeValueAsString(this)

fun <T> T.toJsonNode(): JsonNode = JacksonExtension.jacksonObjectMapper.convertValue(this, JsonNode::class.java)
